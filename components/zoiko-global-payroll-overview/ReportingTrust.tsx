...
const cards = [
  ...
  },
  {
    ...
    items: ["Direction, source of record and versioned\nmapping", "Provider and service model disclosed per\nconnector", "Partial failures, retry and support owner tracked"],
    icon: "/22.png",
  },
  {
    ...
    <section className="w-[1180px] max-w-[1180px] mx-auto px-8 py-20 flex flex-col items-start gap-7">
      ...
      <div className="w-[680px] max-w-[680px] pt-[1.62px] flex flex-col items-start">
        ...
        <div className="w-[640px] max-w-[640px] text-slate-600 text-base leading-6">
          Consolidated views never replace local truth — and trust claims are only made<br/>when there&apos;s documented evidence behind them.
        </div>
      </div>
      ...
      <div className="w-full pt-3.5 bg-white rounded-2xl shadow-[0px_8px_24px_0px_rgba(8,47,73,0.06)] shadow-[0px_1px_2px_0px_rgba(8,47,73,0.04)] outline outline-1 outline-offset-[-1px] outline-slate-100 overflow-hidden">
        <div className="w-full h-64 overflow-hidden">
          <Image src="/net.png" alt="Reporting trust" width={1114} height={260} className="w-full h-full object-cover" />
        </div>
      </div>
        ...
        {cards.map((c) => (
          <div key={c.title} className="w-96 px-6 pt-6 pb-11 bg-white rounded-2xl shadow-[0px_1px_2px_0px_rgba(8,47,73,0.04)] outline outline-1 outline-offset-[-1px] outline-slate-100 flex flex-col gap-3">
            <div className="w-9 h-9 bg-slate-50 rounded-[10px] outline outline-1 outline-offset-[-1px] outline-slate-100 flex items-center justify-center overflow-hidden">
              <Image src={c.icon} alt={c.title} width={32} height={32} className="w-8 h-8 object-contain" />
            </div>
            ...
            <div className="flex flex-col">
              {c.items.map((item, i) => (
                <div key={item} className={`py-1.5 ${i > 0 ? "border-t border-slate-100" : ""}`}>
                  <span className="text-slate-600 text-xs leading-5 whitespace-pre-line">{item}</span>
                </div>
              ))}
            </div>
          </div>
