    ...
    <section className="w-[1180px] max-w-[1180px] mx-auto px-8 py-20 flex flex-col items-start gap-3">
      ...
      <div className="w-[680px] max-w-[680px] pt-[1.62px] flex flex-col items-start">
        ...
        <div className="pt-3.5 text-sky-950 text-4xl font-extrabold leading-10">
          No hidden composite score. Just what&apos;s blocking close.
        </div>
        <div className="w-[640px] max-w-[640px] text-slate-600 text-base leading-6">
          Every jurisdiction sits in one of five readiness states. Every blocker carries an owner, scope, cutoff and next action — never just a red flag.
        </div>
      </div>
      ...
      {/* Exception rows */}
      {exceptions.map((e) => (
        <div key={e.title} className="w-full px-5 py-4 bg-white rounded-xl outline outline-1 outline-offset-[-1px] outline-slate-100 flex items-center gap-4">
          <div className="py-px">
            <div className={`w-1.5 h-full self-stretch rounded-sm ${e.barColor}`} />
          </div>
          <div className="flex-1 flex flex-col gap-1">
            <div className="text-sky-950 text-sm font-bold leading-5">{e.title}</div>
            <div className="flex items-start gap-4 flex-wrap">
              <span className="text-gray-400 text-xs leading-5">Owner: <span className="text-sky-950 font-bold">{e.owner}</span></span>
              <span className="text-gray-400 text-xs leading-5">Scope: <span className="text-sky-950 font-bold">{e.scope}</span></span>
              <span className="text-gray-400 text-xs leading-5">Cutoff: <span className="text-sky-950 font-bold">{e.cutoff}</span></span>
            </div>
          </div>
          <div className="text-sky-700 text-xs font-bold leading-5">{e.action}</div>
        </div>
      ))}
    </section>
  );
